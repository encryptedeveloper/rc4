...
Exemples:
  ...
  %(prog)s "7f7c7b7a79" "secret" --decrypt --input-encoding hex
  ...
  %(prog)s --input-file input.txt --key "clé" --output-file output.txt --drop 2
    ...
    if args.decrypt:
        ...
        try:
            ...
            # === TENTATIVE DECODAGE UTF-8 ===
            try:
                result = result_bytes.decode('utf-8')
            except UnicodeDecodeError:
                result = result_bytes.hex()
                print("[Info] Le résultat ne peut être affiché en UTF-8, affichage en hexadécimal:")
        
        except ValueError as e:
            print(f"Erreur lors du décodage de l'entrée: {e}")
            print(f"Encodage spécifié: {args.input_encoding}")
            print(f"Données d'entrée (premiers 100 caractères): {input_data[:100]}...")
            sys.exit(1)
